//+------------------------------------------------------------------+
//| BrainBot_STEP_C_v2_13_IntraTP_MinProfit.mq5                       |
//| Step C: Execute Class-A using Node-RED areas + gates              |
//| v2.13 - INTRADAY ONLY - no scalp                                  |
//| - SCALP strategy removed entirely                                  |
//| - INTRADAY strategy always active for all TFs: M/W/D/H1/M5        |
//| - TP = InpMinProfitUSD, SL = TP distance / 3                      |
//| - Max lot per trade via per-slot InpMaxLot_S1..S8                  |
//| - Learning multipliers locked at 1.0 (stats tracking only)        |
//|                                                                   |
//| NOTES                                                             |
//| - STRAT_INTRADAY uses M/W/D/H1/M5 ACC breakout + mitigation.      |
//| - Gates/TTL/dedup/telemetry unchanged.                            |
//+------------------------------------------------------------------+
#property strict
#property version   "2.13"
#property description "BrainBot Step C: INTRADAY ONLY - Node-RED parse + Class-A execution + USD sizing + cooldown + dedup"
...
struct SymbolCtx {
   ...
   //=========================
   // LEARNING STATS (per symbol, intraday only)
   //=========================
...
void LearnOnPositionClosed(SymbolCtx &ctx, const bool isIntraday, const double profitTotal, const double plannedRisk)
{
   ...
   if(!isIntraday) return; // scalp removed; only intraday stats tracked
...
void LearnLoad(const string sym, const string strat, LearnStats &ls)
{
   ...
   // Lock all multipliers at 1.0 (adaptive multipliers disabled)
   ls.riskMult  = 1.0;
   ls.tpMult    = 1.0;
   ls.slBufMult = 1.0;
}
...
void LearnSave(const string sym, const string strat, const LearnStats &ls)
{
   ...
   // Save 1.0 for all multiplier keys (adaptive multipliers disabled)
   GVSetD(LearnKey(sym,strat,"riskM"), 1.0);
   GVSetD(LearnKey(sym,strat,"tpM"),   1.0);
   GVSetD(LearnKey(sym,strat,"slM"),   1.0);
}
                 ...
                 const bool isIntraday)
{
   ...
   g_tradesToday++;
   ctx.lastTradeTimeIntra = TimeCurrent();
...
bool CooldownOK_Mode(const SymbolCtx &ctx, const bool isIntraday)
{
   ...
   datetime lastT = ctx.lastTradeTimeIntra;
...
bool DedupCheckOnly(const SymbolCtx &ctx, const string dedupKey, const bool isIntraday)
{
   ...
   string lastKey = ctx.lastDedupKeyIntra;
   if(lastKey == "") return true;

   datetime lastT = ctx.lastDedupTimeIntra;
...
void PrintGateStatus(SymbolCtx &ctx)
{
   ...
   string status = passIntra ? "PASSED" : "BLOCKED";
...
void PrintBlockedReason(SymbolCtx &ctx)
{
   ...
   if(fI) {
      ctx.lastBlockPrint = now;
      Print("STEP_C [", ctx.sym, "] BLOCK REASON: INTRA=", rI);
   }
...
void ProcessSymbol(SymbolCtx &ctx)
{
   ...
   bool intraOpen = HasOpenPositionForMagic(ctx.sym, InpMagicIntra);
